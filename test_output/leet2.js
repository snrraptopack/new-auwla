...
function test_nested_collections() {
  ...
  let registry = {  };
  registry["primes"] = [2, 3, 5];
  registry["fib"] = [1, 1, 2, 3];
  __print("registry-primes", registry["primes"]);
  if (("primes" in registry)) {
    const p = registry["primes"];
    __print(`Primes: ${p}`);
  }
  ...
  for (const key of keys) {
    if ((key in registry)) {
      total_elements = (total_elements + registry[key].length);
    }
  }
